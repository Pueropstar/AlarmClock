...
- 스마트폰을 사용하여 알람 설정, 알람 해제, 알람 종료가 가능하다.
- 스위치를 사용하여 알람을 종료할 수 있으며, 시계 모드와 온도 모드 전환도 가능하다.

# 기능 상세
**1. BlueTooth를 사용한 스마트폰과 라즈베리 파이 사이의 통신**
- 스마트폰과 블루투스 모듈을 블루투스로 연결
- 블루투스 모듈과 라즈베리 파이 간에 UART 통신을 사용하여 특정 명령어를 통해 알람 설정, 알람 취소, 알람 종료 등이 가능하다.
...
**4. 알람 재생**
- 스마트폰에서 설정한 시간이 되면, 코드 내부에 있는 주파수에 맞게 수동 부저에서 음이 재생되며 알람이 울리게 된다.
